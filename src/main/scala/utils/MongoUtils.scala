...
object CRIMEAData extends App{

  //val path = "/home/nkatz/dev/REVEAL-DATA/quantized_features_5categories-small.txt"
  val path = "/home/nkatz/dev/REVEAL-DATA/quantized_features_5categories-large.txt"
  // 0-3, 8-12
  class Interpretation(x: Array[String]) {
    val line = x.mkString(" ")

    val mergeLLEs = true
    // Merge "shrinking", "continuing", "growing" to a single LLE ("continuing")
    // due to the very low number of LLEs of these types
    val toUniqueLLE = (x: String) => {
      if (mergeLLEs && List("shrinking", "continuing", "growing").contains(x)) "continuing" else x
    }
    ...
    val f3 = x(2)
    val f4 = x(3)
    val label = toUniqueLLE(x(8))
    var finalId = ""
    val currentTime = x(9)
    val nextTime = x(11)
    val currentId = s"com${x(10)}"
    val nextId = s"com${x(12)}"
    def narrative = List(s"happensAt(f1($finalId,$f1),$currentTime)",s"happensAt(f2($finalId,$f2),$currentTime)",
      s"happensAt(f3($finalId,$f3),$currentTime)",s"happensAt(f4($finalId,$f4),$currentTime)")
    def annotation = List(s"example(holdsAt($label($finalId),${currentTime.toInt+1}))")
    def tostring = (narrative++annotation).map(_+".").mkString("\n")
  }
  /* Represents a trace of community in time. Applies to continuing or growing communities (not dissolving ones).
   * A trace is simply a list of interpretations referring to the same community. */
  class Trace(trace: List[Interpretation])

  def toMongo(lines: List[Interpretation]) = {
    val newDB = "CRIMEA"
    val mongoClient = MongoClient()
    mongoClient.dropDatabase(newDB)
    val collection = mongoClient(newDB)("examples")
    lines.foreach { x =>
      val entry = MongoDBObject("time" -> x.currentTime.toInt) ++ ("annotation" -> x.annotation) ++ ("narrative" -> x.narrative)
      collection.insert(entry)
    }
  }

  def toFile(lines: List[Interpretation]) = {
    val y = lines map (x => (x.narrative++x.annotation).mkString(" ")) mkString "\n"
    utils.Utils.writeLine(y,"/home/nkatz/Desktop/kernel.txt","overwrite")
  }
  val lines = Source.fromFile(path).getLines.toList.map(_.split(",")).map(x => new Interpretation(x))
  val traces = lines.map { line =>
    lines.foldLeft(line, List[Interpretation]()) { (x,newLine) =>
      val (startPoint, trace) = (x._1,x._2)
      if ((newLine.currentTime,newLine.currentId) == (startPoint.nextTime,startPoint.nextId)) {
        (newLine,trace :+ newLine)
      } else {
        (startPoint, trace)
      }
    }
  }.map(z => z._2)//.filter(k => k.length > 1)


  val _traces = traces.foldLeft(List[List[Interpretation]]()){ (y,x) =>
    val allIds = x.map(_.currentId)
    val uniqueId = if (allIds.nonEmpty) allIds.head else y.head.head.currentId
    val k = x.foldLeft(List[Interpretation]()){(a,z) =>
      z.finalId = uniqueId
      a :+ z
    }
    y :+ k
  ...
  /*
    val _traces = traces.map{ x =>
      val allIds = x.map(_.currentId)
      val uniqueId = allIds.head
      //x.foreach(z => z.finalId = uniqueId)
      val k = x.map(z => setFinalId(z,uniqueId))
      //k.foreach(z => println(z.finalId))
      //println(" ")
      k
    }
  */
  def setFinalId(x: Interpretation, id: String) = {
    x.finalId = id
    x
  }
  ...
  ///*
  utils.Utils.clearFile("/home/nkatz/Desktop/kernel.txt")
  _traces.foreach { x =>
    //x.foreach{z => println(z.finalId)}
    //println(" ")
    val s = x.map(z => z.tostring).mkString("\n")
    //val s = x.map(z => z.line).mkString("\n")
    utils.Utils.writeLine(s+"\n\n","/home/nkatz/Desktop/kernel.txt","append")
  }
  //*/
