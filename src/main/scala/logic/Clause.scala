...
case class Clause(head: PosLiteral = PosLiteral(), body: List[Literal] = Nil, fromWeakExample: Boolean = false,
                  var supportSet: Theory = Theory(), val uuid: Int = scala.util.Random.nextInt() ) extends Expression  with LazyLogging{


  var time = "0"
  var foreignIndices = (0,0,0,0)
  var tempIndices = (0,0,0,0)
  var clauseIndices = scala.collection.mutable.Map[Int , (Int,Int,Int,Int)]()
  var brotherClauses = List[Clause]()
  var test = true
  var actorsResponded = 0
  var foreign = true
  ...
  def recall: Double = tps.toFloat / ( tps + (fns*10))
  ...
  def meanDiff = {
    ...
    //try {
      ...
      // Also sort clauses by length also, so that sorter clauses be preffered over longer ones with the same score
      val bestTwo = (List(this) ++ this.refinements).sortBy { x => (-x.score, x.body.length + 1) }.take(2)

      //val bestTwo = (List(this) ++ this.refinements).sortBy { x => - x.score }.take(2)

      var tempBest = Clause()
      var tempSecondBest = Clause()
      if (bestTwo.length ==2){
        tempBest = bestTwo.head
        tempSecondBest = bestTwo.tail.head
      }
      else if (bestTwo.length==1)  tempBest = bestTwo.head

      val best = tempBest
      val secondBest = tempSecondBest

      //logger.info(s"bestTwo.head:${bestTwo.head.tostring}, bestTwo.tail.head:${bestTwo.tail.head.tostring}")
  ...
  /*  */
  var meanScoreLearningWholeTheories = 0.0
  def updateScoreLearnWholeTs(newMeanScore: Double) = {
    this.meanScoreLearningWholeTheories = newMeanScore
  }

  /* This is used when learning whole theories. This is the mean
   * observed difference in f-score between the current hypothesis
    * and an alternative hypothesis that contains this clause instead
    * of its parent clause.*/
  def meanDiff2 = {
    val bestTwo = (List(this) ++ this.refinements).sortBy { x => (- x.meanScoreLearningWholeTheories, x.body.length+1) }.take(2)
    val (best,secondBest) = (bestTwo.head,bestTwo.tail.head)
    val newDiff = best.meanScoreLearningWholeTheories - secondBest.meanScoreLearningWholeTheories
    val newMeanDiff = ( (previousMeanDiff*previousMeanDiffCount) + newDiff)/(previousMeanDiffCount+1)
    previousMeanDiffCount += 1 // increase the count
    previousMeanDiff = newMeanDiff
    (newMeanDiff,best,secondBest)
  ...
  def generateCandidateRefs: Unit = {
    ...
    //if (candidateList.length == 0) logger.info(s"zero")
    val refinementsSets =
    ...
    flattend.foreach(x => x.parentClause = this)
