...
case class Theory(var clauses: List[Clause] = List() ) extends Expression with LazyLogging {

  val uuid = scala.util.Random.nextInt
  var location : String = ""
  var pause = false
  var altered = false
  ...
  def reScoreRules(example: Example, jep: Jep, globals: Globals, postPruningMode: Boolean = false, targetClass : String): Unit = {
    // If a rule has just been expanded its refinements are empty, so generate new
    if (!postPruningMode) {
      this.clauses foreach (rule => if (rule.refinements.isEmpty) rule.generateCandidateRefs)
    }
